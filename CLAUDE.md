...
本项目集成了 **Serena MCP 工具**（版本 0.1.4），提供强大的代码分析和项目管理功能。
...
- **`write_file`** - 创建或重写文件
- **`create_text_file`** - 创建文本文件（推荐）
- **`edit_file`** - 行级文件编辑
- **`replace_regex`** - 正则表达式替换
- **`list_dir`** - 列出目录内容
- **`list_dir_with_sizes`** - 带文件大小的目录列表
- **`directory_tree`** - 获取目录树结构
- **`find_file`** - 按文件名搜索
- **`get_file_info`** - 获取文件元信息

#### 符号分析工具
- **`find_symbol`** - 查找符号（类、方法、函数等）
- **`find_referencing_symbols`** - 查找引用符号的位置
...
- **`read_memory`** - 读取项目记忆
- **`write_memory`** - 写入项目记忆
- **`delete_memory`** - 删除项目记忆
- **`get_current_config`** - 获取当前配置
- **`check_onboarding_performed`** - 检查项目引导
...
#### 2. 搜索代码
```
find_symbol("class ASRProcessor", "*.py")
search_for_pattern("def.*voice", paths_include_glob="*.py")
```
...
#### 4. 编辑代码
```
replace_symbol_body("def process_audio", "main.py", "新方法体")
insert_after_symbol("class ChatManager", "chat_manager.py", "新方法")
```

### 💡 开发建议

1. **多语言支持**: Serena 支持 Python、TypeScript、C#、Go、Rust、Java 等 15+ 种语言
2. **符号导航**: 使用 `find_symbol` 快速定位类和函数
3. **引用追踪**: 使用 `find_referencing_symbols` 理解代码依赖
4. **智能搜索**: 结合 `search_for_pattern` 和文件名模式搜索
5. **批量操作**: 使用 `read_multiple_files` 和 `replace_regex` 批量处理

### 注意事项

- Serena 会在 `.serena/` 目录保存项目记忆和配置
- 语言服务器会根据项目语言自动启动
- 所有工具都支持异步操作，适合大型项目
## 📖 项目概览
